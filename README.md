...
```
$ docker pull evolveum/midpoint:4.0.2
```
...
- build:
```
$ docker build -t evolveum/midpoint:4.0.2 ./
```
- or
```
$ ./build.sh
...
```
$ docker run -p 8080:8080 --name midpoint evolveum/midpoint:4.0.2
```
- run image on port 8080 with increased heap size:
```
$ docker run -p 8080:8080 -e MP_MEM_MAX='4096M' -e MP_MEM_INIT='4096M' --name midpoint evolveum/midpoint:4.0.2
```
