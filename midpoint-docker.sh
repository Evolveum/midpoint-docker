...
# [help_o] .t. Initial password for the first run. This is not used for the password change once user is created.
# [help_o] 
		initpw)
			preserved_args="${preserved_args} ${2}"
			midPoint_initPw="${2}"
...
# [help_c] .t. Stop environemnt, removing the container objects except volumes and data on "external" filesystem.
# [help_c] .t. See: clean-db, clean
# [help_c] 
	down)
